...
import { getJwtToken, setJwtToken, updateUserInfo, validateAuthNickname } from '../../auth';
...
type ProfileChanges = Partial<Pick<MemberUpdate, 'memberNick' | 'memberPhone' | 'memberAddress' | 'memberImage'>>;

/** Fields in `draft` that differ from what the signed-in user already has. */
const getProfileChanges = (draft: MemberUpdate, current: CustomJwtPayload): ProfileChanges => {
	const changes: ProfileChanges = {};
	if (draft.memberNick !== (current.memberNick ?? '')) changes.memberNick = draft.memberNick;
	if (draft.memberPhone !== (current.memberPhone ?? '')) changes.memberPhone = draft.memberPhone;
	if (draft.memberAddress !== (current.memberAddress ?? '')) changes.memberAddress = draft.memberAddress;
	if (draft.memberImage && draft.memberImage !== current.memberImage) changes.memberImage = draft.memberImage;
	return changes;
};
...
const MyProfile: NextPage = ({ initialValues, ...props }: any) => {
	...
	const updateProfileHandler = useCallback(async () => {
		try {
			...
			// Send only fields the user actually changed. Resending an unchanged
			// memberNick meant an account whose nickname predates the current rule
			// (e.g. a long name generated by social signup) could not save ANY field,
			// not even an address, because every save resubmitted the invalid nick.
			const changes = getProfileChanges(updateData, user);
			if (changes.memberNick !== undefined) {
				const nickError = validateAuthNickname(changes.memberNick);
				if (nickError) throw new Error(nickError);
			}

			const result = await updateMember({
				variables: { input: { _id: memberId, ...changes } },
			});
			const updatedMember = result.data?.updateMember;
			if (updatedMember?.accessToken) {
				setJwtToken(updatedMember.accessToken);
	...
	}, [t, updateData, updateMember, user]);
	...
	const doDisabledCheck = () => {
		...
		// The backend rejects an update with no fields, so a no-op save would error.
		return Object.keys(getProfileChanges(updateData, user)).length === 0;
	};
